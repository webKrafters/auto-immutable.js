import { CLEAR_TAG, DELETE_TAG, GLOBAL_SELECTOR, MOVE_TAG, NULL_SELECTOR, PUSH_TAG, REPLACE_TAG, SET_TAG, SPLICE_TAG } from './constants';
...
export type DeleteCommand<T> = {
    [DELETE_TAG]: Array<keyof T>;
};
/** As in {"@@MOVE": [-/+fromIndex, -/+toIndex, +numItems? ]}. numItems = 1 by default. */
export type MoveCommand = {
    [MOVE_TAG]: [number, number, number?];
};
...
export type ReplaceCommand = {
    [REPLACE_TAG]: BaseType;
};
/** As in {"@@SET": Replacement value} */
export type SetCommand = {
    [SET_TAG]: BaseType | (<V>(currentValue: V) => any);
};
/** As in {"@@SPLICE": [-/+fromIndex, +deleteCount, ...newItems? ]}. numItems = undefined by default. */
export type SpliceCommand = {
    [SPLICE_TAG]: [number, number, ...Array<any>];
};
export type Tag = ClearTag | DeleteTag | MoveTag | PushTag | ReplaceTag | SetTag | SpliceTag;
export type TagCommand<T extends Tag, P extends Value = Value> = T extends ClearTag ? ClearCommand : T extends DeleteTag ? DeleteCommand<P> : T extends MoveTag ? MoveCommand : T extends PushTag ? PushCommand : T extends ReplaceTag ? ReplaceCommand : T extends SetTag ? SetCommand : T extends SpliceTag ? SpliceCommand : never;
export interface AccessorPayload {
    [propertyPath: string]: Atom;
}
...
export type UpdatePayload<T> = T | ClearTag | ClearCommand | DeleteCommand<T> | MoveCommand | PushCommand | ReplaceCommand | SetCommand | SpliceCommand | Value | Partial<{
